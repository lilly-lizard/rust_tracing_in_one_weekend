// TODO remove all Arc and call cornell_box() inside each thread
...
fn cornell_box() -> (HittableList, Camera, Background) {
    ...
    world.list.push(Box::new(AxisRectangle::new_xz(0.0, 555.0, 0.0, 555.0, 0.0, white.clone())));
    world.list.push(Box::new(AxisRectangle::new_xz(0.0, 555.0, 0.0, 555.0, 555.0, white.clone())));
    world.list.push(Box::new(AxisRectangle::new_xy(0.0, 555.0, 0.0, 555.0, 555.0, white.clone())));
    world.list.push(Box::new(AxisRectangle::new_xz(213.0, 343.0, 227.0, 332.0, 554.0, light))); // normal light
    //world.list.push(Box::new(AxisRectangle::new_xz(113.0, 443.0, 127.0, 432.0, 554.0, light))); // large light
...
fn random_scene() -> (HittableList, Camera, Background) {
    ...
    // ground
    let ground_material = Arc::new(Lambertian::new(Box::new(texture::CheckerTexture::new_from_colors(
        Vec3::new(0.2, 0.3, 0.1),
        Vec3::new(0.9, 0.9, 0.9),
        10.0,
    ))));
    world.list.push(Box::new(Sphere::new(Vec3::new(0.0, -1000.0, 0.0), 1000.0, ground_material)));
        ...
        for b in -11..11 {
            ...
            if (center - Vec3::new(4.0, 0.2, 0.0)).length() > 0.9 {
                if choose_mat < 0.8 {
                    ...
                    world.list.push(Box::new(Sphere::new(center, 0.2, sphere_material)));
    ...
    world.list.push(Box::new(Sphere::new(Vec3::new(0.0, 1.0, 0.0), 1.0, material_1)));

    let material_2 = Arc::new(Lambertian::new_from_color(Vec3::new(0.4, 0.2, 0.1)));
    world.list.push(Box::new(Sphere::new(Vec3::new(-4.0, 1.0, 0.0), 1.0, material_2)));

    let material_3 = Arc::new(Metal::new(Vec3::new(0.7, 0.6, 0.5), 0.0));
    world.list.push(Box::new(Sphere::new(Vec3::new(4.0, 1.0, 0.0), 1.0, material_3)));
...
fn ray_color(ray: &Ray, world: &HittableList, background: &Background, recursive_depth: i32) -> Vec3 {
    ...
    if let Some(hit_res) = world_hit {
        ...
        let emitted = hit_res.material().emitted(&hit_res);
        ...
        match scatter_result {
            ...
            Some(scatter) => {
                ...
                return emitted + scatter.albedo * hit_res.material().scattering_pdf(ray, &hit_res, &scatter.ray) * color / scatter.pdf
            },
        }
    }
...
fn main() {
    ...
    println!("image render + write took {}.{}s to complete", duration.as_secs(), duration.subsec_millis());
}
